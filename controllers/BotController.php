                    ...
                    if(!empty($item['message']) && empty($item['message']['is_echo'])){
                        ...
                        try
                        {
                            ...
                            if(empty($user) && is_numeric($command)){
                                ...
                                if(!empty($user)){
                                    //unbind all users who have this bot id
                                    User::updateAll(['bot_user_id' => ''],['bot_user_id' => $senderId]);

                                    //bit just current user
                                    $user->bot_user_id = $senderId;
                                    $user->update();

                                    if($user->role_id == Constants::ROLE_NEW){
                                        $response = "Вы привязали аккаунт к пользователю {$user->getFullName()} \n";
                                        $response .= "Теперь вы можете получать уведомления о заявках, отвечать на вопросы поддержки, дополнять заявки прямо в Messenger. Дождитесь сообщения об изменения статуса вашей заявки и следуйте инструкциям. Вы также можете посмотреть свои заявки в системе обратной связи: https://ctc.gl/profile";
                                    }else{
                                        $response = "Вы привязали аккаунт к пользователю {$user->getFullName()} \n";
                                ...
                                if($user->role_id == Constants::ROLE_NEW){
                                    ...
                                    //if wants to talk
                                        ...
                                        //if has some opened tickets
                                            ...
                                            //if waiting for answers but in dialog state was an error
                                            if(!empty($awaitingAnswers) && !in_array($user->botLastDialogState(),[Constants::BOT_NEED_ANSWER,Constants::BOT_NEED_CONFIRM_ANSWER])){
                                            ...
                                            try{
                                                if($user->botLastDialogState() == Constants::BOT_SESSION_INIT){
                                                    if(count($opened) > 1){
                                                        ...
                                                        $response = "Произошла ошибка инициализации диалога";
                                                        $user->botEndDialog();
                                                    }
